...
export function useAnalyzeTitles(wordCount, readTime) {
    const { article } = useBlogConfig();
    const topWordCount = computed(() => replaceValue(article?.analyzeTitles?.topWordCount || '字数：{{value}} 个字', wordCount.value));
    const topReadTime = computed(() => replaceValue(article?.analyzeTitles?.topReadTime || '预计：{{value}} 分钟', readTime.value));
    const inlineWordCount = computed(() => replaceValue(article?.analyzeTitles?.inlineWordCount || '{{value}} 个字', wordCount.value));
    const inlineReadTime = computed(() => replaceValue(article?.analyzeTitles?.inlineReadTime || '{{value}} 分钟', readTime.value));
    const wordCountTitle = computed(() => article?.analyzeTitles?.wordCount || '文章字数');
    const readTimeTitle = computed(() => article?.analyzeTitles?.readTime || '预计阅读时间');
    const authorTitle = computed(() => article?.analyzeTitles?.author || '本文作者');
    const publishDateTitle = computed(() => article?.analyzeTitles?.publishDate || '发布时间');
    const lastUpdatedTitle = computed(() => article?.analyzeTitles?.lastUpdated || '最近修改时间');
    const tagTitle = computed(() => article?.analyzeTitles?.tag || '标签');
    return {
        topWordCount,
        topReadTime,
        inlineWordCount,
